...
def create_queue(client, name, fifo):
    ...
    try:
        if fifo == 'true':
            ...
            response = client.create_queue(QueueName=name+'.fifo',
...
def enqueue (sqs, queue_url, message, message_group_id=''):
    ...
    try:
        ...
        if response['ResponseMetadata']['HTTPStatusCode'] != 200:
            print ('[lambda_handler] Error putting message to the queue (Error code - %d)' % response['ResponseMetadata']['HTTPStatusCode'])
            print ('[lambda_handler] Response for send_message - HTTPStatusCode: %d (Message: %s)' % (response['ResponseMetadata']['HTTPStatusCode'], message))
        return response
